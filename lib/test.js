...
events.on('bedrock-cli.test.configure', function(command) {
  command
    // TODO: should probably be --framework mocha
    // and then register frameworks with the config and check to see if
    // your framework is in the config list before running anything
    .option(
      '--mocha',
      'Only run Mocha, no other test frameworks.')
    .option(
      '--mocha-test <files>',
...
function runMocha(state, callback) {
  ...
  mocha.run(function(failures) {
    ...
    var cancel = config.cli.command.mocha || config.cli.command.mochaTest;
