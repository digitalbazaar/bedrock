...
var logger = require('./loggers').get('app');
...
api.hashJsonLd = function(obj, frame, callback) {
  ...
  async.waterfall([
    function(callback) {
      ...
      if(!('@context' in obj)) {
        obj = tools.clone(obj);
        obj['@context'] = config.constants.CONTEXT_URL;
...
api.signJsonLd = function(obj, options, callback) {
  ...
  async.waterfall([
    ...
    function(data, callback) {
      // get created date
      var created = tools.w3cDate(date);
      ...
      signer.on('finish', function() {
        var signature;
        try {
          signature = signer.sign(privateKey.privateKeyPem, 'base64');
        } catch(e) {
        ...
        // set signature info
        var signInfo = {
          '@context': config.constants.SECURITY_CONTEXT_V1_URL,
...
/**
 * Verifies the signature on a JSON-LD object containing a signature.
 *
 * @param obj the JSON-LD object to verify the signature on.
 * @param options the options to use:
 *   publicKey the public key to verify with.
 *   [publicKeyOwner] the Identity that owns the public key.
 * @param callback(err, verified) called once the operation completes.
 */
api.verifyJsonLd = function(obj, options, callback) {
  var publicKey = options.publicKey;

  // FIXME: update any source code that provides the publicKey directly
  if('publicKeyPem' in options) {
    logger.warn('bedrock.security.verifyJsonLd no longer supports ' +
      'providing the publicKey directly to the function. Use \'options\' ' +
      'instead.');
  ...
  if(!('creator' in obj.signature)) {
    return callback(new BedrockError(
      'Could not verify signature on object. Signature does not have ' +
      '\'creator\'.', 'bedrock.security.InvalidSignature'));
  }

  async.auto({
    fetchPublicKey: function(callback) {
      if(publicKey) {
        return callback(null, publicKey);
      }

      // FIXME: re-enable after fixing other code
/*
      request(obj.signature.creator, function(err, res, body) {
        // FIXME: can't just JSON.parse, use try/catch
        callback(err, JSON.parse(body));
      });*/
    },
    fetchPublicKeyOwner: ['fetchPublicKey', function(callback, results) {
      publicKey = results.fetchPublicKey;
      ...
      // FIXME: re-enable after fixing other code
      /*request(publicKey.owner, function(err, res, body) {
        // FIXME: can't just JSON.parse, use try/catch
        callback(err, JSON.parse(body));
      });*/
    }],
    verifySignature: ['fetchPublicKeyOwner', function(callback, results) {
      // FIXME: re-enable after fixing other code
      /*
      // ensure public key is owned by public key owner
      var keys = results.fetchPublicKeyOwner.publicKey;
      var keyOwnerCheckPassed = false;
      if(!Array.isArray(keys)) {
        keys = [keys];
      }
      // check all owner keys to find signing key
      for(var i = 0; i < keys.length; i++) {
        var key = keys[i];
        if('id' in key && key.id === publicKey.id) {
          if('owner' in key && key.owner === publicKey.owner) {
            keyOwnerCheckPassed = true;
            i = keys.length;
          }
        }
      }

      if(!keyOwnerCheckPassed) {
        return callback(new BedrockError(
          'The key used to sign the message is not claimed by the owner. ' +
          'Possible digital signature forgery detected.',
          'bedrock.security.SignatureForgeryDetected'));
      }*/
      ...
      _getSignatureData(obj, function(err, data) {
        ...
        verifier.on('finish', function() {
          var verified = verifier.verify(
            publicKey.publicKeyPem, signInfo.signatureValue, 'base64');
  ...
  async.waterfall([
    ...
    function(key, iv, callback) {
      ...
      cipher.on('finish', function() {
        ...
        var msg = {
          '@context': config.constants.CONTEXT_URL,
