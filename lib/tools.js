...
/**
 * Assigns a getter to the value of a config key. When the config key is read, 
 * the getter will execute and compute the configured value. This is 
 * particularly useful for config values that depend on other config values; 
 * it removes the need to reset such a value when its dependencies change.
 * 
 ...
 * 
 * @param namespacedConfig the parent object.
 * @param key the property name to set.
 * @param fnOrExpression a lodash template or a function used to compute the 
 *          value of `key`.
 * @param [options] options to use:
 *          locals: local variables use for fnOrExpression.
 */
api.computedConfig = function(namespacedConfig, key, fnOrExpression, options) {
  options = options || {};
  ...
  var _isSet = false;
  var _value;
  Object.defineProperty(namespacedConfig, key, {
    configurable: true,
    enumerable: true,
    get: () => {
      if(_isSet) {
      ...
      return fnOrExpression(options.locals || config);
    },
    set: (value) => {
      _isSet = true;
      _value = value;
