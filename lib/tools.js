...
// check if a path has any elements
...
api.Config.prototype.set = function(path, value) {
  if(typeof path === 'object') {
    Object.keys(path).forEach((key) => {
      _.set(this.object, key, path[key]);
    });
    return;
  }
...
api.Config.prototype.setComputed = function(path, fnOrExpression, options) {
  ...
  if(typeof path === 'object') {
    ...
    Object.keys(path).forEach((key) => {
      this.setComputed(key, path[key], options)
    });
    return;
  }
  ...
  Object.defineProperty(target, targetKey, {
    ...
    get: () => {
      ...
      return fnOrExpression(options.locals || this.options.locals || config);
    },
    set: (value) => {
      _isSet = true;
      _value = value;
