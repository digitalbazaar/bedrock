  ...
  if(!existing) {
    ...
    wrapper.log = function(level, msg, ...meta) {
      if(arguments.length === 1) {
        // level = {..._logger._bedrockMeta, ...level};
...
container.init = async () => {
  ...
  WorkerTransport.prototype.log = function(info, callback) {
    if(this.silent) {
  ...
  for(const cat in config.loggers.categories) {
    const logger = new winston.createLogger({
      transports: [
        new WorkerTransport({level: lowestLevel, levels, category: cat})
      ]
    });
    const _existingLogger = container.get(cat);
    if(_existingLogger) {
