...
container.get = container.add = function(id) {
  var logger;
  if(!container._init) {
    // logging not initialized, return empty logger reference that will
    // be updated once initialization occurs
    logger = container.loggers[id] = {};
    return logger;
  }
  logger = container._get.apply(container, arguments);
  if(logger instanceof winston.Logger) {
    var wrapper = {};
    wrapper.__proto__ = logger;
    logger = container.loggers[id] = logger;
  }
  return logger;
};
...
container.init = function(callback) {
  ...
  if(cluster.isMaster) {
    ...
    var transports = {
      console: new winston.transports.Console(config.loggers.console),
      app: new winston.transports.File(config.loggers.app),
      access: new winston.transports.File(config.loggers.access),
      error: new winston.transports.File(config.loggers.error),
      email: new WinstonMail(config.loggers.email)
    };
        ...
        async.forEach(filenames, function(filename, callback) {
          try {
            ...
            if(config.environment !== 'development') {
              var uid = config.app.user.userId;
  ...
  WorkerTransport.prototype.log = function(level, msg, meta, callback) {
    ...
    } else if(tools.isObject(meta) && 'error' in meta) {
