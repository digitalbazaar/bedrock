...
api.start = function(options, done) {
  ...
  async.auto({
    ...
    init: ['beforeInit', function(callback) {
      _parseCommandLine();
      _loadConfigs();
      _configureLoggers();
      _configureWorkers();
...
api.setProcessUser = function() {
  ...
  if(config.environment !== 'development' && process.setgid && process.setuid) {
    process.setgid(config.core.user.groupId);
    process.setuid(config.core.user.userId);
  }
...
api.runOnce = function(id, fn, options, callback) {
  ...
  function runOnce(msg) {
    ...
    var _callback = function(err) {
      process.send({type: 'bedrock.runOnce', id: id, done: true, error: err});
    };
...
function _runMaster(options) {
  ...
  process.title = config.core.masterTitle + (args ? (' ' + args) : '');
  ...
  cluster.on('exit', function(worker, code) {
    // if the worker suicided, it was intentional, so exit the process
    if(worker.suicide) {
      logger.info(
...
function _runWorker(startTime, done) {
  ...
  // set 'ps' title
  var args = process.argv.slice(2).join(' ');
  process.title = config.core.workerTitle + (args ? (' ' + args) : '');
  ...
  async.auto({
    ...
    configure: ['cliReady', function(callback, results) {
    ...
    init: ['configure', function(callback) {
      events.emit('bedrock.init', callback);
    }],
    start: ['init', function(callback) {
      // set process user
      api.setProcessUser();
