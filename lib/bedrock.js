...
function _runMaster(options) {
  ...
  // TODO: use config system not environment for this
...
function _runWorker(startTime, callback) {
  var logger = loggers.get('app');
  ...
  async.auto({
    ...
    init: ['cliReady', function(callback, results) {
    ...
    start: ['init', function(callback, results) {
      if(results.init === false) {
        return callback(null, false);
      }
      // set process user
      api.setProcessUser();
    ...
    ready: ['start', function(callback, results) {
    ...
    started: ['ready', function(callback, results) {
...
// TODO: remove 'testRunner', use bedrock.runOnce
