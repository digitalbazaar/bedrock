...
api.start = function(options, done) {
  ...
  loggers.init(function(err) {
    ...
    if(cluster.isMaster) {
      ...
      var server = express();
      app.server = server;
      events.emit('bedrock-server.init');
      ...
      // init limiter
      events.emit('bedrock-limiter.init');
      limiter.init(app, function() {
        events.emit('bedrock-limiter.ready');
      });
      ...
      events.emit('bedrock-server.configure.logger');
      ...
      events.emit('bedrock-server.configure.bodyParser');
      server.use(express.bodyParser());
      events.emit('bedrock-server.configure.cookieParser');
      ...
      events.emit('bedrock-server.sessionStore.init');
      var database = require(__dirname + '/bedrock/database');
      database.createSessionStore(express, function(err, store) {
        ...
        events.emit('bedrock-server.sessionStore.ready');
        configure();
      });

      function configure() {
        events.emit('bedrock-server.configure.session');
        if(config.environment !== 'down') {
        ...
        events.emit('bedrock-server.configure.compress');
        server.use(function(req, res, next) {
          var i = -1;
        ...
        events.emit('bedrock-server.configure.static');
        for(var i = config.server.static.length - 1; i >= 0; --i) {
          ...
          if(cfg.file) {
            ...
            server.use(cfg.route, express.static(
              p, config.server.staticOptions));
          }
        }
        // do not cache non-static resources
        events.emit('bedrock-server.configure.cache');
        server.use(function(req, res, next) {
        ...
        events.emit('bedrock-server.configure.router');
        server.use(server.router);
        events.emit('bedrock-server.configure.routes');
        // all custom error handlers to be added later
        events.emit('bedrock-server.configure.errorHandlers');
        ...
        events.emit('bedrock-server.start');
        async.auto({
          httpsListen: function(callback) {
            async.forEach(config.server.bindAddr, function(addr, next) {
              var logger = loggers.get('app');
              logger.debug('starting HTTPS server on %s:%d',
                addr, config.server.port);
              https_.on('error', function(err) {throw err;});
              events.emit('bedrock-server.https.listen', {
                address: addr,
                port: config.server.port
              });
              https_.listen(config.server.port, addr, function() {
                events.emit('bedrock-server.https.listening', {
                  address: addr,
                  port: config.server.port
                });
                next();
          ...
          httpListen: function(callback) {
            async.forEach(config.server.bindAddr, function(addr, next) {
              var logger = loggers.get('app');
              logger.debug('starting HTTP server on %s:%d',
          ...
          loadModules: ['httpsListen', 'httpListen', function(callback) {
            ...
            events.emit('bedrock.modules.load');
            var modules;
            ...
            moduleApis.forEach(function(moduleApi) {
              ...
              logger.warning('loading module with deprecated API: "' +
                moduleApi.name + '"');
              events.on('bedrock.start', function(callback) {
          ...
          // TODO: w/new event API, all modules should listening for
          // bedrock.start (or other events) before doing init
          // FIXME: move events.emit to inside async.auto
          events.emit('bedrock.start', function(err) {
            if(err) {
              events.emit('bedrock.error', err);
              return;
            }
            ...
            events.emit('bedrock.ready', function(err) {
              if(err) {
                events.emit('bedrock.error', err);
                return;
              }
