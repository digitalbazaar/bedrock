...
var api = {};
// TODO: move most of these to their own modules, use require() to
// make use of their APIs
api.config = config;
api.docs = docs;
api.events = events;
api.jsonld = jsonld;
  ...
  loggers.init(function(err) {
    ...
    async.auto({
      ...
      loadModules: ['beforeLoadModules', function(callback, results) {
        ...
        // any other module
        // TODO: rename to bedrock-validation
        api.validation = require('bedrock-validation');
        ...
        // any other module
        // TODO: don't attach to 'app', just use
        // require('bedrock-express').app in other modules
        api.app.server = require('bedrock-express').app;
...
function _startWorker(state, testRunner) {
  ...
  worker.on('message', function(msg) {
    ...
    if(msg.id in state.runOnce.ids) {
      // already ran/running
      return;
    }
