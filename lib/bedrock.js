...
api.start = function(options, done) {
  ...
  // add program options
  if(options.program) {
    if('version' in options.program) {
      program.version(options.program.version);
    } else {
      program.version(api.version);
    }
    var opts = options.program.options || [];
    for(var i = 0; i < opts.length; ++i) {
      program.option(opts[i]);
    }
  } else {
    program.version(api.version);
  }
    ...
    _.each(cats, function(cat) {
      ...
      _.each(transports, function(transport) {
        if(transport.indexOf('-') === 0) {
          ...
          tName = transport;
          tIndex = catTransports.indexOf(tName);
          if(tIndex === -1) {
            catTransports.push(tName);
          }
        }
