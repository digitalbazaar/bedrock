...
api.start = function(options, done) {
  ...
  program
    ...
    .option('--log-only <module>',
      'Only log events from the specified modules.')
  ...
  async.auto({
    ...
    initLoggers: ['init', function(callback) {
    ...
    run: ['initLoggers', function(callback) {
      if(cluster.isMaster) {
        _runMaster(options);
...
function _runWorker(startTime, done) {
  ...
  async.auto({
    ...
    adminInit: ['configure', function(callback) {
      events.emit('bedrock.admin.init', callback);
    }],
    init: ['adminInit', function(callback) {
      // set process user
      api.setProcessUser();
