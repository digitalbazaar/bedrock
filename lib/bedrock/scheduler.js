  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      bedrock.events.on(EVENT_SCAN, function(event) {
        bedrock.logger.verbose('got job scan event', event);
        var options = {};
  ...
  async.until(function() {return done;}, function(loopCallback) {
    ...
    async.auto({
      ...
      checkResult: ['runJob', function(callback, results) {
        ...
        var logFn = error ? bedrock.logger.error : bedrock.logger.verbose;
        logFn(
    ...
    bedrock.logger.verbose('job worker (' + workerId + ') finished.');
