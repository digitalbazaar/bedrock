  ...
  async.waterfall([
    ...
    function(callback) {
      ...
      for(var i = 0; i < bedrock.config.scheduler.concurrency; ++i) {
        bedrock.events.emit({type: EVENT_SCAN, details: {}});
      }
      callback();
    }
  ], callback);
...
function _getNextJobSchedule(job, options) {
  ...
  if(interval.length === 2) {
    ...
    start = new Date(interval[1]).getTime();
    duration = interval[2];
  }
  ...
  if(options.update) {
    ...
    if(repeats !== -1) {
      ...
      job.schedule += ['', bedrock.tools.w3cDate(), duration].join('/');
    } else {
      ...
      checkResult: ['runJob', function(callback, results) {
        ...
        if(due === null) {
          return bedrock.db.collections.job.remove(
            ...
            bedrock.db.writeOptions, callback);
        }

        // update job with new schedule if schedule hasn't already changed
        // and this worker hasn't expired
        var update = {
          $set: {
