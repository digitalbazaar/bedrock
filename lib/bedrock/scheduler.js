...
/**
 * Schedules a new job. The job must have a unique id and a type. It may
 * also specify how often to run the job. Jobs may be scheduled that do not
 * have defined types, however, they will never be executed by this particular
 * scheduler (other schedulers may execute them).
 *
...
api.schedule = function(job, callback) {
  if(!(job.id && job.type)) {
    return callback(new BedrockError(
      'Could not schedule job; no job id and/or type was specified.',
  ...
  if(job.type in jobTypes) {
    defaults = bedrock.tools.extend({}, defaults, jobTypes[job.type].defaults);
  }
