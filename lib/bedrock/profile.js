...
api.resolveProfilename = function(name, callback) {
  bedrock.db.collections.profile.findOne(
    {$or: [{id: bedrock.db.hash(name)}, {'profile.psaSlug': name}]},
    {'profile.id': true},
    function(err, result) {
      if(!err && result) {
 ...
 * has no 'id' but has a 'psaSlug' property, then the 'id' will be generated
 * from 'psaSlug'. If there is no 'psaSlug' property, then a unique slug and
      ...
      async.auto({
        hashPassword: function(callback) {
          if(profile.psaHashedPassword === true) {
            ...
            bedrock.security.createPasswordHash(profile.psaPassword, callback);
          }
        },
        ...
        // an ID was given
        var result = null;
        var generateSlug = !profile.psaSlug;
        ...
        async.until(function() {return result !== null;}, function(callback) {
          ...
          if(generateSlug) {
            profile.psaSlug = bedrock.tools.uuid();
            delete profile.id;
          }
