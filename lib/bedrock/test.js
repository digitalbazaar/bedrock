...
function runTests() {
  ...
  if(process.env.TEST_ENV === 'unit') {
    ...
    var seleniumJar = '/tmp/sv-selenium/selenium-server-standalone.jar';
    var chromeDriver = '/tmp/sv-selenium/chromedriver';
    ...
    if(!fs.existsSync(seleniumJar) || !fs.existsSync(chromeDriver)) {
      console.log('Selenium binaries not found. Run \'npm run install-selenium\'');
      process.exit(1);
    }
    ...
    agent.get('http://localhost:4444/').end(function(err, status) {
      ...
      selenium.stdout.on('data', function(data) {
        ...
        if(/Started SocketListener on/.test(data)) {
          ...
          var debugging =
            process.env.BEDROCK_DEBUG.split(',').indexOf('test') !== -1;
