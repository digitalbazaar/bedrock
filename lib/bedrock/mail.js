    ...
    function(callback) {
      ...
      async.forEach(events, function(info, callback) {
        ...
        bedrock.events.on(info.type, function(event) {
          bedrock.logger.verbose('mailer handling event: ' + event.type);
          ...
          async.waterfall([
            ...
            if(err) {
              ...
              bedrock.logger.verbose('sent email details', details);
            }
          });
        });
        callback();
...
api.send = function(id, vars, callback) {
  ...
  if(entry.disabled) {
    bedrock.logger.debug('not sending email, template "' + id + '" disabled.');
    return callback();
  }
  ...
  try {
    ...
    return callback(new BedrockError(
      ...
      MODULE_NS + '.TemplateCompileError', {
  ...
  try {
    ...
    return callback(new BedrockError(
      ...
      MODULE_NS + '.TemplateProcessingError', {
        filename: entry.filename
      }, ex));
  }
