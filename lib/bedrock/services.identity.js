...
function addServices(app, callback) {
  ...
  app.server.get(idPath + '/dashboard', ensureAuthenticated, _handler);
  app.server.get(idPath + '/settings', ensureAuthenticated, _handler);
...
function _getCredentials(req, res, next) {
  ...
  if(!req.query.callback ||
    ...
    ~req.query.callback.indexOf('https://'))) {
    return next(new BedrockError(
    ...
    get: function(req, res, callback) {
      ...
      validate('identityCredentialsQuery', query, function(err) {
        ...
        if(req.query.authorize === 'true') {
          // TODO: actually process query to create response
          var response = {'@context': 'https://w3id.org/identity/v1'};
          if('id' in query) {
            query.id = req.user.identity.id;
          }

          // TODO: sign response (include domain)

          // use response as resource
