  ...
  app.server.post(idPath,
    ...
    function(req, res, next) {
      ...
      if(req.body.signingKey) {
        identity.signingKey = req.body.signingKey;
      }

      // FIXME: also add support in schema, UI, and elsewhere
      /*
    ...
    get: function(req, res, callback) {
      ...
      validate('identityCredentialsQuery', query, function(err) {
        ...
        if(req.query.authorize === 'true') {
          ...
          bedrock.identity.signJsonLdAsIdentityService(identity, {
            domain: req.query.domain,
            nonce: '' // FIXME
          }, function(err, signedIdentity) {
            // use identity as resource
            return callback(null, signedIdentity);
          });
        }
