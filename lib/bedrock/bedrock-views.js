...
var bedrockExpress = require('./bedrock-express');
...
bedrock.events.on('bedrock-express.configure.static', function() {
  ...
  bedrockExpress.app.use(function(req, res, next) {
...
function configure() {
  ...
  bedrockExpress.app.engine('html', function(path, options, callback) {
    var view;
    try {
  ...
  bedrockExpress.app.set('view engine', 'html');
  bedrockExpress.app.set('views', paths);

  if(bedrock.config.environment === 'down') {
    // system down output for all requests
    bedrockExpress.app.all('*', function(req, res, next) {
  ...
  */
  bedrock.config.website.views.routes.forEach(function(route) {
    ...
    bedrockExpress.app.get(options.path, function(req, res, next) {
      api.getDefaultViewVars(req, function(err, vars) {
        if(err) {
          return next(err);
        }
...
// add error handler
bedrock.events.on('bedrock-express.configure.errorHandlers', function() {
  bedrockExpress.app.use(function(err, req, res, next) {
    if(!err) {
      return next();
    }
