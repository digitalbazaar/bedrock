...
api.resolveIdentityIdentifier = function(identifier, callback) {
  ...
  api.resolveIdentitySlug(identifier, function(err, record) {
    if(err) {
      return callback(err);
    }
    if(!record) {
    ...
    // arrayify result
    callback(null, [record.identity.id]);
  });
};
...
api.setIdentityStatus = function(actor, id, status, callback) {
  async.waterfall([
    function(callback) {
      // FIXME: check permissions here ... always require admin ... should
      // this be done with a simpler function?
      api.checkActorPermissionForObject(
        actor, {id: id},
        PERMISSIONS.IDENTITY_ADMIN, PERMISSIONS.IDENTITY_ADMIN, callback);
    },
    function(callback) {
      bedrock.db.collections.identity.update(
...
api.setIdentityPassword = function(actor, identity, callback) {
  ...
  async.auto({
    checkPermission: function(callback) {
      api.checkActorPermissionForObject(
        actor, identity,
        PERMISSIONS.IDENTITY_ADMIN, PERMISSIONS.IDENTITY_EDIT, callback);
    },
    checkPassword: ['checkPermission', function(callback) {
      if('sysPassword' in identity) {
        return api.verifyProfilePassword(identity, callback);
      }
      callback(null, null);
    }],
    checkPasscode: ['checkPermission', function(callback) {
      if('sysPasscode' in identity) {
        return api.verifyProfilePasscode(identity, callback);
      }
      callback(null, null);
    }],
...
api.sendIdentityPasscodes = function(identities, usage, callback) {
  ...
  // create event
  var event = {
    type: 'common.Identity.passcodeSent',
    details: {
      usage: usage,
      identities: [],
      email: null
...
/**
 * Sets the Identity's Roles from the identity['sysRole'] array of RoleIDs.
 *
 * @param actor the Identity performing the action.
...
api.setIdentityRoles = function(actor, identity, callback) {
  async.waterfall([
    function(callback) {
      // FIXME: check permission ... should it just be identity admin only?
      api.checkActorPermission(
        actor, PERMISSIONS.IDENTITY_ADMIN, PERMISSIONS.IDENTITY_ADMIN,
        callback);
    },
    function(callback) {
      bedrock.db.collections.identity.update(
      ...
      bedrock.db.collections.identity.findOne(
        ...
        function(err, result) {
          ...
          if(!result) {
            ...
            result = result.identity.sysRole;
          }
          bedrock.permission.getRoles(result, callback);
        });
    },
 ...
 *
 * @param actor the Identity to check.
...
api.checkActorPermissionForObject = function(
  actor, object, permissionSpecial, permissionNormal, comparator, callback) {
  var length = arguments.length;
  if(length === 5) {
    callback = comparator;
    comparator = null;
  ...
  async.waterfall([
    ...
    function(callback) {
      api.checkActorPermission(actor, permissionSpecial, function(err) {
        ...
        if(err) {
          if(!comparator) {
            return api.checkActorOwnsObject(actor, object, callback);
          }
          return api.checkActorOwnsObject(actor, object, comparator, callback);
        }
        callback();
      });
    }
...
api.addIdentityPublicKey = function(actor, publicKey, privateKey, callback) {
  ...
  async.waterfall([
    function(callback) {
      bedrock.identity.checkActorPermissionForObject(
        actor, publicKey,
  ...
  async.auto({
    ...
    hashPassword: ['checkDuplicate', function(callback) {
      if(identity.sysHashedPassword === true) {
        ...
        callback(null, identity.sysPassword);
      }
      else {
    ...
    hashPasscode: ['generatePasscode', function(callback, results) {
      if(identity.sysHashedPasscode === true) {
        ...
        callback(null, identity.sysPasscode);
      }
      else {
