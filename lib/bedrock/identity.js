...
api.createIdentity = function(actor, identity, callback) {
  async.waterfall([
    function(callback) {
      api.checkPermission(
        actor, PERMISSIONS.IDENTITY_CREATE,
        {resource: identity, translate: 'owner'}, callback);
    },
    function(callback) {
  ...
  async.waterfall([
    ...
    function(identity, meta, callback) {
      api.checkPermission(
        ...
        {resource: identity, translate: 'owner'},
        function(err) {
 ...
 *          [translate](resources, callback(err, identifiers)) either a
 *            function to map the given resources to a list of identifiers to
...
api.checkPermission = function(actor, permission, options, callback) {
  // grant permission; no actor (full admin mode)
  if(!actor) {
    return callback(null, []);
  }
  async.auto({
      ...
      bedrock.db.collections.identity.findOne(
        ...
        function(err, result) {
          ...
          if(!result) {
            ...
            result = result.identity.sysResourceRole;
          }
    ...
    cacheTable: ['getActorRoles', function(callback, results) {
      ...
      actor.sysPermissionTable = bedrock.permission.createPermissionTable(
        results.getActorRoles);
      callback();
    }],
    checkPermission: ['cacheTable', function(callback) {
      bedrock.permission.checkPermission(
        actor.sysPermissionTable, permission, options, callback);
    }]
  }, function(err, results) {
    callback(err, results.checkPermission);
  });
};
  ...
  async.waterfall([
    ...
    function(key, callback) {
      api.checkPermission(
        ...
        {resource: key, translate: 'owner'}, function(err) {
          callback(err, key);
        });
    },
    function(key, callback) {
...
function _createIdentity(identity, callback) {
  async.auto({
    ...
    setDefaults: ['checkDuplicate', function(callback) {
      ...
      if(!('sysResourceRole' in identity)) {
        identity.sysResourceRole = [];
        defaults.roles.forEach(function(role) {
          var r = {
            sysRole: role.sysRole
          };
          // default to identity as resource
          if('resource' in role) {
            r.resource = role.resource.slice();
          }
          else {
            r.resource = [identity.id];
          }
          identity.sysResourceRole.push(r);
        });
      }
