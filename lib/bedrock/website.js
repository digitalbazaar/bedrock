...
/**
 * Makes a single page REST handler for the server.
 *
 * FIXME: Add docs.
 *
 * @param options the handler options.
 *   getResource(req, res, callback(err, data)): get resource data
 ...
 *   template: template file name (default: 'main.html')
 */
api.singlePageRestHandler = function(options, callback) {
  options = options || {};
  return function(req, res, next) {
    function _json() {
      ...
      options.getResource(req, res, function(err, data) {
        if(err) {
          return next(err);
        }
        res.json(data);
    ...
    function _html() {
      async.auto({
        vars: function(callback) {
          api.website.getDefaultViewVars(req, callback);
        },
        ...
        resource: ['identity', function(callback) {
          if(options.templateNeedsResource) {
            return options.getResource(req, res, callback);
          }
          callback();
        }],
