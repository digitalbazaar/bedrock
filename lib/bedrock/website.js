...
function configureServer(app, callback) {
  ...
  // add swig as the default template engine, include special options
  app.server.engine('html', function(path, options, fn) {
    // pass extra swig options to consolidate
    options = bedrock.tools.extend({}, options, {
      autoescape: false,
      cache: (bedrock.config.environment !== 'development') ? true : false,
      loader: swigLoaders.multipath({
        base: bedrock.config.website.views.paths
      })
    });
    return consolidate.swig(path, options, fn);
  });
