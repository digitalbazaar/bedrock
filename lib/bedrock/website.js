 ...
 */
api.makeResourceHandler = function(options, callback) {
  options = options || {};
    ...
    function _html() {
      async.auto({
        ...
        identity: ['vars', function(callback, results) {
          ...
          // use identity from session
          results.vars.identity = req.user.identity;
          results.vars.clientData.identity = req.user.identity;
          callback();
        }],
