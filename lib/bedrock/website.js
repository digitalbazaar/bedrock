...
// custom handlers for view vars
var viewVarsHandlers = [];
...
api.init = function(app, callback) {
  ...
  async.waterfall([
    ...
    function(callback) {
      // register to add root services when bedrock is ready to start
      bedrock.events.on('bedrock.start', function() {
        // let other modules do last minute configuration
        bedrock.events.emit('bedrock.website.services.add');
        addServices(app);
      });
      callback();
    }
...
api.getDefaultViewVars = function(req, callback) {
  ...
  async.auto({
    ...
    custom: ['getIdentities', function(callback) {
      async.forEachSeries(viewVarsHandlers, function(handler) {
        handler(req, vars, callback);
      }, callback);
    }]
  }, function(err) {
    callback(err, vars);
...
/**
 * Registers a custom handler to be called when fetching default view vars.
 *
 * @param handler(req, vars, callback) the handler to add.
 */
api.addViewVarsHandler = function(handler) {
  viewVarsHandlers.push(handler);
};
